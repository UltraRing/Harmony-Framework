/// @description Script
    ...
    //Collect
    if(player_collide_object(COLLISION.MAIN) && obj_player.state != player_state_knockout)
    {
		//Play the sound
		sound_ring_pan_play()
	...
	if(ringloss)
	{
		...
		//Collision detection
		var hc = collision_get_distance(x + 8 * sign(x_speed), y, sign(x_speed) ? COLLISION_MODE.LEFT_WALL : COLLISION_MODE.RIGHT_WALL, plane, false);
		var vc = collision_get_distance(x, y + 8 * sign(y_speed), sign(y_speed) ? COLLISION_MODE.FLOOR : COLLISION_MODE.CEILING, plane, sign(y_speed));
